  ...
  it('should not resize the component outside the parent node if `parent` prop is true and `lock-aspect-ratio` is set', function (done) {
    ...
    wrapper.vm.$nextTick(() => {
      ...
      syn.drag(
        ...
        {
          from: { pageX: fromX, pageY: fromY },
          to: { pageX: fromX + 300, pageY: fromY },
          duration: 1000
        },
        function () {
          wrapper.vm.$children[0].$nextTick(() => {
            expect($el.style.left).to.equal('0px')
            expect($el.style.top).to.equal('0px')
            expect(parseFloat($el.style.width) <= 375).to.be.true
            expect(parseFloat($el.style.height) <= 500).to.be.true
            done()
          })
        }
      )
